...
interface Props {
    data: BalanceOpsDataV1;
    onInteract?: (log: RawInteractionLog) => void;
    onComplete?: (result: any) => void;
    readOnly?: boolean;
}

export const BalanceOpsComponentV1: React.FC<Props> = ({
    data,
    onInteract = () => { },
    onComplete = () => { },
    readOnly
    ...
    const rawConfig = (data as any).interaction?.config ?? (data as any).content?.interaction?.config;
    ...
    // Init Effect
    useEffect(() => {
        onInteract({ type: 'mount', timestamp: Date.now() });
    ...
    const applyOp = (op: any) => {
        if (readOnly || isolated || submitted) return;

        // Log interaction
        onInteract({
            type: 'click',
            payload: { action: 'apply_operation', op: op.op_id, value: op.value },
            timestamp: Date.now()
        });
    ...
    const triggerSubmit = () => {
        ...
        onInteract({
            type: 'submit',
            payload: { isCorrect, steps: history.length },
            timestamp: Date.now()
        });
        ...
        if (isCorrect && autoAdvance !== false) {
            ...
            // For incorrect, we might want to let them keep trying unless it's an exam mode
            // But for this template, usually isolation = pass. If they click submit without isolation, it's incorrect.
            onComplete(resultData);
        }
    };
    ...
    const getPrompt = () => {
        ...
        return (data as any).content || "Solve for " + variableName;
    };

    if (!config) return <div className="p-4 text-red-500">Invalid Configuration</div>;

    return (
        <div className="flex flex-col items-center py-4 w-full bg-white rounded-2xl shadow-sm border border-slate-100 p-6 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-2xl mb-8">
                {dynamicOps.map((op, idx) => (
                    <button
                        key={idx}
                        onClick={() => applyOp(op)}
                        disabled={readOnly || isolated || submitted}
                        ...
                        {!submitted && (
                            <button
                                onClick={() => {
                                    setIsolated(false);
                                    setHistory([]);
                                    onInteract({ type: 'click', payload: { action: 'reset' }, timestamp: Date.now() });
